class AppData
  ...
  def url_schemes
    {
      twodays: "2 Days=twodays://x-callback-url/add?page={{#{twodayspage}}}&text={{Check out '+t+': '+location.href+'}}",
      buffer: "Buffer=bufferapp://?u={{'+location.href+'}}",
      cleanlinks: "Clean Link=clean-links://x-callback-url/clean?url={{'+location.href+'}}",
      clear: "Clear=clearapp://task/create?listName={{#{clearlist}}}&listPosition=&taskName={{Check out '+t+': '+location.href+'}}&taskPosition=Top",
      ...
      due: "Due=due://x-callback-url/add?title={{Check out '+t+': '+location.href+'}}",
      ...
      taskagent: "Task Agent=taskagent://x-callback-url/action?list={{#{taskagentlist}}}&task={{Check out '+t+': '+location.href+'}}",
      texttool: "Encode Link=texttool://x-callback-url/transform?text={{'+location.href+'}}&method=encode&x-success={{launch:}}",
      things: "Things=things:add?title={{Check out '+t+'}}&notes={{'+location.href+'}}",
